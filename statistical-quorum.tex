...
\def\plaintitle{Improving credibility and efficiency of decision-making within democratic communities via statistical quorum}
%\def\plainauthor{First Author, Second Author, Third Author,
%  Fourth Author, Fifth Author, Sixth Author}
\def\emptyauthor{}
\def\plainkeywords{Decision making; voting; democracy; quorum; credibility; statistical quorum.}
...
\title{Improving credibility and efficiency of decision-making \\ within democratic communities via statistical quorum}
...
We found that the community was able to use the online system to simultaneously improve the credibility and efficiency
of their decision-making process.
\end{abstract}

\category{H.5.3.}{Information Interfaces and Presentation (e.g. HCI)}{Computer-supported cooperative work}
...
\item We implemented a web application \emph{PeerMind},
...
% Security is out of the scope of this work. We assume here that users and their identities are known and are managed
% by a community's admin. There are many communities where their decision making is public with public minutes and this
% is who the app is currently targeting.
...
through regular features of the application: how many and who made discussion items, comments, motions
(proposed decisions on discussion items), and vote tallies per motion.

For the baseline, we extracted similar information from council minutes: how long did the councils last, how many
discussions were opened, how many comments by which members were made, how many motions were proposed,
and vote tallies for them.
...
We built a full-fletched mobile-friendly web applicationusing Meteor web framework~\cite{meteor}.
...
and proposed motions with voting on the right. All content inputs fields accept rich text content in WYSWYG manner,
...
which moderators summarize into summary points. At some point an user proposes a motion which can be
further discussed through comments. Once a moderator deems that motion is ready to be voted upon
...
results of the vote in real time. Results are shown on a chart depicting average score through time
as seen in Figure~\ref{fig:results}. Users can see both the current average score of all votes (score
larger than 0 means that a motion is passing) and the credibility of current result. A community can decide
...
The rationale for displaying the current results during voting is to give users feedback how the vote
...
After the vote passes the statistical quorum, a moderator can close voting and a discussion item.
...
and display summary points of current discussion and what motions are being voted on. Through that the
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth]{figures/peermind.png}
\caption{User interface of PeerMind web application. One can see three column structure, with commenting
section on the left, summary points in the middle, and proposed motions with voting on the right.}
\label{fig:peer-mind}
\end{figure}
...
remove themselves from the population of the community for the vote in question), or to not vote (default), which
...
\caption{Results of a vote in PeerMind web application. Results are shown as an average score through time of the vote.
...
In Figure~\ref{fig:survey-comparison} we see questions and results of the survey we made at the end of each semester.

We can see that in fall members were more satisfied with the decision making in the community and only how understandable
is the whole process received a decrease. We attribute this to the fact that the use of the web application is
a novelty and requires more effort in educating members than a traditional decision making. Moreover, decision
making in person allows one to observe others how they are participating and learn from them, instead of
being alone using the application.
