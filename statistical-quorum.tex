...
We describe decision credibility in more detail in Section~\ref{sec:statistical-quorum}\todo{Hm, SIGCGI template seems to not display section numbers. So how useful is to reference a section number?}.
...
\lim_{t\rightarrow \infty}\Pr\left(N(t)\leq \hat{K_i}\right) & = \sum_{n=0}^{\hat{K_i}} \frac{(\lambda_0 \cdot K_i/N_i)^n}{n!(\ln{\gamma})^n}e^{-\lambda_0 \frac{K_i/N_i}{\ln{\gamma}}}\\
& \geq 95\% \nonumber
\end{align}
...
% describe why 0.90% is a reasonable statistical quorum and how to determine one for your community
...
\end{itemize} 
\end{description}
Setting $\lambda (t) = \lambda_0 {(\gamma)}^{-\frac{t}{K_i/N_i}}$ satisfies these constraints while still allowing for the exact rate to be tunable.
...
&= \lambda_0 \frac{K_i/N_i}{\ln{\gamma}}.
\end{align*}
Thus, 
\begin{align*}
\lim_{t\rightarrow \infty}\Pr\left(N(t) = n\right) = \frac{(\lambda_0 \cdot K_i/N_i)^n}{n!(\ln{\gamma})^n}e^{-\lambda_0 \frac{K_i/N_i}{\ln{\gamma}}},
\end{align*}
and
\begin{align*}
\lim_{t\rightarrow \infty}\Pr\left(N(t)\leq \hat{K_i}\right) & = \sum_{n=0}^{\hat{K_i}} \frac{(\lambda_0 \cdot K_i/N_i)^n}{n!(\ln{\gamma})^n}e^{-\lambda_0 \frac{K_i/N_i}{\ln{\gamma}}}.
