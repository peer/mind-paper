...
\def\emptyauthor{}
\def\plainkeywords{Decision making; voting; democracy; quorum; legitimacy; statistical quorum.}
...
We propose a novel \emph{statistical quorum}, derived using first principles, as one metric the community can use to focus
...
councils over the course of 16 weeks.  We found that the community was able to use the web application to simultaneously
...
Web application was developed using Meteor web framework~\cite{meteor} as a mobile-friendly web application.
...
Each user can open and propose new discussion items. Each discussion item has then three basic sections,
as seen in Figure~\ref{fig:peer-mind}: commenting section on the left, summary points in the middle,
...
can open voting. Users vote through a ballot seen in Figure~\ref{fig:ballot}.
